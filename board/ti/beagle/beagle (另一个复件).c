...
#define RSLT_WIDTH             1024
#define RSLT_HEIGHT				768
...
void dss_init(void)
{
	...
	for(i = 0; i<(((RSLT_HEIGHT/2)-(beagle_height/2))*RSLT_WIDTH)*2; i+=2) {
	...
	for(i = 0; i<(RSLT_HEIGHT- (offset/2/RSLT_HEIGHT))*RSLT_WIDTH*2; i+=2) {
		*((unsigned short *)(0x80500000 + i + offset)) = 0xffffff;
	}
