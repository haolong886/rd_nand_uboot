...
#define FB_ADDR					gImage_16bpp
#define RGB24					0x00000013
#define RGB16					0x0000000d
#define GFXBURSTSIZE_4			0x0
#define GFXBURSTSIZE_8			0x40
#define GFXBURSTSIZE_16			0x80
#define GFX_FIFO_THRESHOLD_HI	(0xaff << 16)
#define GFX_FIFO_THRESHOLD_LO	(0x2ff)
void dss_init(void)
{

//	memcpy(FB_ADDR, gImage_logo_kernel, sizeof(gImage_logo_kernel));
	*((uint *) DISPC_GFX_BA0) = FB_ADDR; //frame buffer addr
	*((uint *) DISPC_GFX_BA1) = FB_ADDR; //frame buffer addr
	*((uint *) DISPC_GFX_POSITION) = 0x00ff009a; //image position y:x
	*((uint *) DISPC_GFX_SIZE) = 0x00ff02cc; //image size y:x
	*((uint *) DISPC_GFX_FIFO_THRESHOLD) = GFX_FIFO_THRESHOLD_HI | GFX_FIFO_THRESHOLD_LO;
	
//	*((uint *) DISPC_GFX_ATTRIBUTES) = 0x00000093;  //RGB24
	*((uint *) DISPC_GFX_ATTRIBUTES) = RGB16 | GFXBURSTSIZE_16;  //RGB16
...
//--------------------------
	...
	case REVISION_XM_C:
	default:
//added by haolong 2012/03/13, this is to set DVI_PUP signal
		//set twl4030 gpio2 as output
		twl4030_i2c_read_u8(TWL4030_CHIP_GPIO, &val,
                             TWL4030_BASEADD_GPIO + 0x3);
		val |= 0x4;
		twl4030_i2c_write_u8(TWL4030_CHIP_GPIO, val,
			     TWL4030_BASEADD_GPIO + 0x3);
		//set twl4030 gpio2 output 1
		twl4030_i2c_read_u8(TWL4030_CHIP_GPIO, &val,
...
//added by haolong for Setting VAUX2 to 1.8V for EHCI PHY
	case UESTC_RD_REV1:
		printf("UESTC_RADAR Rev 2.1 NAND version\n");	
